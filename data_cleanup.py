...
class DataIntegrity:
    ...
    def merge_duplicate_target_rows(self, duplicate_column: str, target_columns: list) -> None:
        ...
        for d in tqdm(dups):
            ...
            tmp2 = self.source.loc[to_keep].copy()
            tmp2.update(temp)
    ...
    def remove_empty_rows(self, target_columns: list) -> None:
        self.source.dropna(subset=target_columns, how='all', inplace=True)

    def save(self, filename_sdf: str, filename_csv: str, column_names=None) -> None:
        """
        ...
        """
        if column_names:
            if "Molecule" not in column_names:
                self.source = self.source[column_names + ["Molecule"]]
            else:
                self.source = self.source[column_names]
...
if __name__ == "__main__":
    filenames = ["benchmark_datasets/tox21/tox21_10k_data_all.sdf",
                 "benchmark_datasets/tox21/tox21_10k_challenge_test.sdf"]
    for filename in filenames:
        only_filename = filename.split('/')[-1].split('.')[0]
        di = DataIntegrity(filename)
        try:
            di.clean_columns(['FW'])
        except KeyError as e:
            print(f"Unable to find 'FW' column in {filename} dataset")
        target_columns = ['SR-HSE',
                          'NR-AR', 'SR-ARE', 'NR-Aromatase', 'NR-ER-LBD', 'NR-AhR', 'SR-MMP',
                          'NR-ER', 'NR-PPAR-gamma', 'SR-p53', 'SR-ATAD5', 'NR-AR-LBD']
        di.change_types(target_columns, float)
        di.merge_duplicate_target_rows('SMILES', target_columns)
        di.remove_empty_rows(target_columns)
        dataset_columns = ['SMILES', 'SR-HSE', 'NR-AR', 'SR-ARE', 'NR-Aromatase', 'NR-ER-LBD', 'NR-AhR', 'SR-MMP',
                           'NR-ER', 'NR-PPAR-gamma', 'SR-p53', 'SR-ATAD5', 'NR-AR-LBD']
        di.save(f'benchmark_datasets/tox21/{only_filename}_duplicates_merged.sdf',
                f'benchmark_datasets/tox21/{only_filename}_duplicates_merged.csv', column_names=dataset_columns)
